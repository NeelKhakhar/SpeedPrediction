...
def MILSTM(n_steps, n_features, n_outputs):
    ...
    model.add(LSTM(5*n_features, activation='tanh', return_sequences= True, activity_regularizer=regularizer, kernel_initializer=RandomUniform(minval=-.0001, maxval=0.0001), input_shape=(n_steps, n_features)))
    ...
    model.add(Conv1D(filters= 2*n_features, kernel_size=8, kernel_regularizer=regularizer, padding= 'same'))
    model.add(Flatten())
    
    # model.add(Dense(128, activation='relu'))
    # model.add(Dense(128, activation='relu'))
    model.add(Dense(128, activation='relu', kernel_initializer='glorot_uniform', activity_regularizer=regularizer))
