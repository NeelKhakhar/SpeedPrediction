...
from tensorflow.keras.layers import LSTM, Reshape, Flatten, Conv1D, Input, Add, Lambda, Concatenate, Bidirectional
...
def MILSTM(n_steps, n_features, n_outputs):
    ...
    model.add(Conv1D(filters= 8, kernel_size=8, kernel_regularizer=regularizer, padding= 'same', input_shape=(n_steps, n_features)))
    model.add(Conv1D(filters= 8, kernel_size=8, kernel_regularizer=regularizer, padding= 'same'))
    model.add(Conv1D(filters= 6, kernel_size=8, kernel_regularizer=regularizer, padding= 'same'))
    model.add(Conv1D(filters= 6, kernel_size=8, kernel_regularizer=regularizer, padding= 'same'))
    model.add(LSTM(50, activation='relu', return_sequences= True, activity_regularizer=regularizer, kernel_initializer=RandomUniform(minval=-.0001, maxval=0.0001)))
    ...
    opt = Adam(clipnorm = 50, learning_rate= 1e-3)
